...
from .models import Category,  AdImage,Ad ,User ,Comment ,FeaturedAd, FeaturedAdHistory,PendingFeaturedAd ,Notification
...
def index(request):
    ...
    parent_categories = Category.objects.filter(parent_category__isnull=True).prefetch_related(
        Prefetch(
            'subcategories',
            queryset=Category.objects.prefetch_related(
                Prefetch(
                    ...
                    queryset=Ad.objects.filter(
                        is_approved=True,
                        featuredad__isnull=True
                    ),
                    to_attr='approved_ads'
                )
            ),
            to_attr='subcategories_list'
        )
    )
...
def menu(request):
    ...
    featured_ads = Ad.objects.filter(
        ...
        featuredad__featured_start_date__lte=timezone.now(),
        featuredad__featured_expiry_date__gte=timezone.now()
    ).prefetch_related('featuredad')  # Prefetch reverse relation if applicable
    ...
    # Get all categories (if needed for template)
    categories = Category.objects.all()

    # Render the template with the context data
    return render(
        request,
        'base/post_ad_categories.html',  # Adjust path as needed
        {
            'categories': categories,
            'featured_products': featured_ads,
            'parent_categories': parent_categories,  # Added this in case the template needs it
        }
    )
...
def ad_form(request, category_id):
    """Submit a free ad (pending admin approval)."""
    category = get_object_or_404(Category, id=category_id)
    categories = Category.objects.all()
    show_success = False  # Initialize here
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            try:
                ...
                show_success = True
                # Keep form instance for display
                form = AdForm()  # Reset form but keep using AdForm
            except Exception as e:
                messages.error(request, f"Error: {str(e)}")
                form = AdForm(request.POST, request.FILES)  # Rebind form with data
        else:
            messages.error(request, "Invalid form data. Please check fields.")
            form = AdForm(request.POST, request.FILES)  # Rebind form with data
    else:
        form = AdForm()  # Use correct form class for GET requests
...
def search_ads(request):
    ...
    # Category search (include subcategories)
    if category_id:
        try:
