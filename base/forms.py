...
# ── STEP 1 — Signup: email + full_name + password only ───────────────────────
class SignupForm(forms.ModelForm):
    ...
    class Meta:
        model = User
        fields = ['email', 'full_name']   # stripped down — no phone/bio/location/avatar
        widgets = {
    ...
    def save(self, commit=True):
        ...
        user.is_active = True           # active immediately — no email block
        user.email_verified = False
        user.profile_completed = False  # must complete Step 2 before posting
        if commit:
            user.save()
        return user


# ── STEP 2 — Complete Profile: phone + location + avatar + bio ───────────────
class CompleteProfileForm(forms.ModelForm):

    class Meta:
        model = User
        fields = ['phone_number', 'location', 'avatar', 'bio']
        widgets = {
    ...
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        # All fields mandatory in Step 2
    ...
    def clean_bio(self):
        ...
        if not bio:
            raise ValidationError("Please write a short bio.")
        if len(bio) < 10:
    ...
    def save(self, commit=True):
        user = super().save(commit=False)
        user.profile_completed = True   # unlock full access
        if commit:
            user.save()
        return user
